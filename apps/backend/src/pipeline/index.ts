...
export interface RunPipelineOptions {
  ...
  model: string;
  /** Trace ID for telemetry correlation. Generated by the Orchestrator per user message. */
  traceId?: string;
}
...
export async function runPipeline(opts: RunPipelineOptions): Promise<PipelineDecision> {
  const { input, sessionId, workspacePath, model, traceId } = opts;
  ...
  if (traceId) {
    telemetryBus.emitEvent({
      ...
      status: 'success',
      metadata: {
        looksLikeAction: gate.looksLikeAction,
        score: gate.score,
  ...
  const classify = await classifyIntent(input, context, model, sessionId, traceId);
  ...
  const extracted = await extractParams(input, classify.intent, context, workspacePath, model, sessionId, traceId);

  // Step 4 — Execution router (tool forcing + validation)
  const decision = routeExecution(extracted, classify, sessionId, traceId);
